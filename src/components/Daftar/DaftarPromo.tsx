  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setSelectedPromo(updatedPromos);
        setIsLoading(false);
      }
    } catch (error) {
        ...
        <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
          <div className="max-w-full overflow-x-auto">
            <table className="w-full table-auto">
              ...
              {!isLoading && selectedPromo ? (
                <tbody>
                  {selectedPromo.map((order, key) => {
                      ...
                      <tr key={key}>
                        ...
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <p className="capitalize">
                            {order.minTrans ? formatToIDR(order.minTrans) : "-"}
                          </p>
                        </td>
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          {order.menuB ? (
                            <div className="flex h-full flex-col items-start gap-2 ">
                              {order.menuB?.slice(0, 2).map((cat) => (
                                <p
                                  key={cat.id}
                              ...
                              {order.menuB?.length > 2 && (
                                <>
                                  <p
                                    data-tooltip-id={`tooltip-${order.id}`}
                                    className="rounded-lg bg-gray-100 px-2 py-1 capitalize"
                                  >
                                    +{order.menuB?.length - 2} lainnya
                                  </p>
                                  <Tooltip
                                    id={`tooltip-${order.id}`}
                                    place="bottom"
                                  >
                                    <div className="flex h-full flex-col items-start gap-2 ">
                                      {order.menuB?.slice(2).map((cat) => (
                                        <p
                                          key={cat.id}
                                          className="rounded-lg bg-gray-100 px-2 py-1 capitalize text-black"
                                        >
                                          {cat.name}
                                        </p>
                                      ))}
                        ...
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <p>
                            {order.discType && order.discAmount
                              ? order.discType === "percentage"
                                ? order.discAmount + "%"
                                : formatToIDR(order.discAmount)
                              : "-"}
                          </p>
                        </td>
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
