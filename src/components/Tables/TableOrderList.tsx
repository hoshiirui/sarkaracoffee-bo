...
const TableOrderListPageContent = () => {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Apakah anda yakin ingin menghapus pesanan?")) {
      ...
      try {
        ...
        if (error) {
          ...
          ToastSuccess("Berhasil menghapus pesanan!");
          fetchData();
        }
      } catch (error) {
  ...
  const filterOrder = () => {
    const filteredOrders = defaultOrderList.filter((order) => {
      const statusMatch =
        selectedFilter === "all" || order.status === selectedFilter;
      const nameMatch =
        !searchName ||
        order.nama.toLowerCase().includes(searchName.toLowerCase());
  ...
  useEffect(() => {
    filterOrder();
  }, [searchName, selectedFilter, defaultOrderList]);

  return (
    <>
      <DefaultLayout>
        <Breadcrumb pageName="Order Masuk" />
...
export default TableOrderListPageContent;
